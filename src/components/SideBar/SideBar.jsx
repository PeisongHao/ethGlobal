...
import { FlowWalletContext } from "../../context/WalletContext"; // ✅ 改成 FlowWalletContext
...
import { FaHome, FaShoppingCart, FaWallet, FaInfoCircle, FaSearch, FaHandHoldingUsd } from "react-icons/fa"

export default function SideBar({ currentPage }) {
  const { flowAddress, flowBalance, connectFlowWallet } = useContext(FlowWalletContext); // ✅ Flow 变量
      ...
      <SidebarBody className="flex-1 min-h-0 overflow-y-auto pl-10 pr-8 pt-6 space-y-6">
        ...
        <div className="space-y-6">
          ...
          <SidebarSection className="space-y-2">
            ...
            <SidebarItem
              onClick={() => { if (flowAddress) { navigate("/profile") } else { connectFlowWallet() } }} // ✅ Flow
            ...
            <SidebarItem
              href="/lending"
              current={currentPage === "lending"}
              className={clsx(
                "flex items-center gap-4 text-xl font-semibold px-6 py-4 rounded-2xl bg-white/50 backdrop-blur-sm hover:shadow-sm transition duration-200",
                currentPage === "lending" ? "bg-orange-50 hover:bg-orange-100" : "hover:bg-fuchsia-50"
              )}
            >
              <FaHandHoldingUsd data-slot="icon" className={clsx("w-7 h-7", currentPage === "lending" ? "text-orange-500" : "text-fuchsia-500")} />
              <SidebarLabel className={clsx("text-xl font-bold", currentPage === "lending" ? "text-orange-500" : "text-neutral-700")}>Lending</SidebarLabel>
            </SidebarItem>
        ...
        {flowAddress && (
          <>
            <div className="text-lg font-bold mb-1">
              ...
              <span className="text-pink-500">{Number(flowBalance).toFixed(2)}</span>
            </div>
            <div className="text-lg font-bold">
              <span className="text-neutral-700">Flow Balance:</span>{" "}
              <span className="text-pink-500">{Number(flowBalance).toFixed(2)}</span>
            </div>
          </>
        )}
