...
import { FlowWalletContext } from "../../context/WalletContext"; // ✅ 用 FlowWalletContext
import styles from "./TopBar.module.css";

import { IoMdTrendingUp } from "react-icons/io";

export default function TopBar() {
  const { flowAddress, flowBalance, connectFlowWallet } = useContext(FlowWalletContext);

  // Function to shorten wallet address (e.g., "0xabc...1234")
  const shortenAddress = (address) => {
    if (!address) return "Please connect Flow wallet";
      ...
      <div>
        {flowAddress ? (
          <div className="text-xl font-semibold px-10 py-3 rounded-2xl bg-gradient-to-r from-[#ff03ea] to-[#ff7700] text-white">
            {shortenAddress(flowAddress)} ({Number(flowBalance).toFixed(2)} FLOW)
          </div>
        ) : (
          <button
            className="text-xl font-semibold px-12 py-3 rounded-2xl bg-gradient-to-r from-[#ff03ea] to-[#ff7700] hover:brightness-110 transition duration-200 text-white"
